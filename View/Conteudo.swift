...
struct Content: View {
    ...
    @State var storageMeals: ManagerStorageMeals = ManagerStorageMeals(storageMeals: [])
    
    var body: some View {
        VStack {
            if contentOfMethods.name == "GET" {
                MethodGet(selectedIndex: $selectedIndex, typeMethod: contentOfMethods.name)
                ...
                MethodDelete( storageMeals: $storageMeals)
            }
        }
    }
